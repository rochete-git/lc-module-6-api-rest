...
export const CharacterDetailsContainer: React.FunctionComponent = (props) => {
  ...
  const handleLoadCharacterDetails = async () => {
    ...
    const apiCharacterDetails = await api.getCharacterDetailsGraphQL(id);
    const apiCharacterComment = await api.getCharacterCommentGraphQL(id);
    setExistComment(apiCharacterComment.id > 0)
    setCharacterDetails(mapCharacterDetailsFromApiToVm(apiCharacterDetails, apiCharacterComment.comment));
  };
  ...
  const handleSaveComment = async (id: number, comment: string) => {
    ...
    const success = await api.saveCharacterCommentGraphQL(apiCharacterComment, existComment);
    if (success) {
      history.goBack();
    } else {
